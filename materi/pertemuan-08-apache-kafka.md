...
## Konteks: Sistem Peringatan Dini Bencana Kota Surabaya

Pemkot Surabaya sedang mengembangkan **Sistem Peringatan Dini Bencana Kota (SPDBK)** — platform terpadu yang mengintegrasikan data dari ratusan sensor tersebar di seluruh kota untuk mendeteksi dan merespons bencana (banjir, kebakaran, gempa, angin kencang) secara real-time.

Tim kamu ditunjuk sebagai **Kafka Architect & Data Engineer** untuk sistem ini. Kamu menerima briefing dari Dinas BPBD Surabaya dan developer senior yang sudah membuat scaffolding awal, dan kamu harus merancang serta membangun pipeline berbasis Kafka.

> 🚨 **Situasi Kritis:** Dalam 2 minggu, sistem akan memasuki fase **Proof of Concept (PoC)** yang akan dipresentasikan kepada Walikota. Keputusan anggaran Rp 2 miliar bergantung pada hasil PoC ini. Tim kamu harus menyelesaikan semua deliverable!

---

### Briefing Teknis dari Developer Senior

Developer senior meninggalkan catatan berikut sebelum cuti:

```
Catatan untuk tim baru:

Saya sudah setup Docker Compose dan struktur folder. Tapi ada 3 masalah
yang belum selesai dan butuh keputusan arsitektur:

MASALAH 1 — TOPIC DESIGN:
Saya tidak yakin berapa partisi yang tepat untuk setiap topic. 
Ada 200 sensor, 5 tipe bencana, dan punya rencana 4 consumer berbeda.
Tolong tentukan dan JELASKAN alasannya (bukan hanya angkanya!).

MASALAH 2 — BOTTLENECK CONSUMER:
Consumer "notifikasi-warga" saya yang lama hanya bisa proses 50 event/detik,
tapi sensor mengirim 200 event/detik saat puncak. Tolong identifikasi 
apa yang harus dilakukan dan implementasikan solusinya.

MASALAH 3 — DURABILITAS vs BIAYA:
Tim infrastruktur tanya: berapa lama data sensor harus disimpan di Kafka?
Saya tidak tahu menjawabnya tanpa konteks bisnis. Tolong analisis dan 
buat rekomendasi yang bisa dipertanggungjawabkan.

- Pak Budi (Developer Senior, sedang cuti)
```
...
#### Deliverable 1: Dokumen Arsitektur (30%)

Buat dokumen arsitektur (`ARSITEKTUR.md`) yang menjawab **3 masalah dari Pak Budi** dengan dasar teknis yang kuat:

**Untuk MASALAH 1 (Topic Design):**
- Identifikasi semua topic yang dibutuhkan sistem (minimal 4 topic)
- Tentukan jumlah partisi setiap topic dan **jelaskan formula/alasan pemilihannya**
- Tentukan replication factor dan **jelaskan trade-off dengan biaya storage**
- Tentukan retention policy per topic dan **justifikasi berdasarkan kebutuhan bisnis** (bukan angka default!)
- Tentukan key strategy (field apa yang dijadikan key) dan **jelaskan implikasi ordering-nya**

**Untuk MASALAH 2 (Consumer Bottleneck):**
- Analisis: apakah bottleneck di partisi atau di logika consumer?
- Jelaskan langkah-langkah yang kamu ambil untuk menyelesaikannya
- Jelaskan apa risiko jika kamu scale out consumer melebihi jumlah partisi

**Untuk MASALAH 3 (Retention Policy):**
- Buat matriks keputusan: untuk setiap topic, berapa lama data disimpan dan mengapa
- Hitung estimasi storage yang dibutuhkan dengan asumsi: 200 sensor × 1 event/detik × ukuran event ~500 byte
- Rekomendasikan apakah butuh log compaction untuk topic manapun

> ℹ️ **Panduan:** Dokumen arsitektur yang baik berisi KEPUTUSAN dengan ALASAN, bukan hanya konfigurasi. Bayangkan kamu harus meyakinkan Pak Budi kenapa pilihanmu lebih baik dari alternatifnya.

---

#### Deliverable 2: Implementasi Pipeline (50%)

Bangun pipeline Kafka lengkap dalam folder `spdbk-pipeline/` dengan struktur:

```
spdbk-pipeline/
├── docker-compose.yml
├── setup_topics.sh           # Script buat semua topic sesuai desain Deliverable 1
├── producers/
│   ├── sensor_banjir.py      # Simulasi sensor ketinggian air (threshold based)
│   ├── sensor_kebakaran.py   # Simulasi sensor panas + asap
│   └── [sensor lain sesuai desain kamu]
├── consumers/
│   ├── alerting_engine.py    # Deteksi kondisi bencana & kirim alert
│   ├── data_archiver.py      # Simpan semua data ke file JSONL
│   └── analytics_live.py    # Statistik rolling per tipe sensor
└── README.md
```

**Persyaratan teknis minimum per komponen:**

| Komponen | Persyaratan |
| -------- | ----------- |
| **Producer** | Real data simulation (bukan random murni), gunakan key strategy sesuai desain, minimal 2 tipe sensor berbeda |
| **Consumer alerting** | Multi-level alert (WARNING / DANGER / CRITICAL) berbasis threshold, masing-masing level punya logika berbeda |
| **Consumer archiver** | Manual offset commit, simpan ke file `.jsonl`, handle graceful shutdown |
| **Consumer analytics** | Consumer group BERBEDA dari alerting, hitung rolling statistics (min/max/avg) per sensor ID |
| **Semua consumer** | Harus bisa di-restart dan melanjutkan dari offset terakhir (tidak dari awal) |

---

#### Deliverable 3: Skenario Uji Kegagalan (20%)

Ini bagian yang tidak bisa dikerjakan dengan AI — kamu harus **betul-betul menjalankan dan mengamati** apa yang terjadi.

Lakukan 3 skenario berikut, **rekam/screenshot output**, dan tulis analisis singkat (1 paragraf per skenario):

**Skenario A — Consumer Crash:**
1. Jalankan producer + 2 instance `alerting_engine.py` dalam group yang sama
2. Saat keduanya aktif, matikan PAKSA salah satu (Ctrl+Z atau `kill -9`)
3. Amati: apakah consumer yang tersisa mengambil alih partisi yang ditinggalkan?
4. Amati: ada event yang "terlewat" tidak diproses?

> Pertanyaan yang harus dijawab: Berapa lama waktu rebalancing? Apakah ada **"stop the world"** period saat rebalancing? Bagaimana cara meminimalkannya?

**Skenario B — Producer Overload:**
1. Jalankan producer dengan delay `time.sleep(0)` (kirim secepat mungkin)
2. Jalankan consumer dengan `time.sleep(0.1)` per event (sengaja lambat)
3. Monitor consumer lag dengan `kafka-consumer-groups.sh --describe` setiap 30 detik selama 3 menit
4. Rekam nilai LAG dari waktu ke waktu

> Pertanyaan yang harus dijawab: Pada titik mana sistem "tidak aman"? Apa yang akan terjadi jika retention period terlampaui sebelum consumer sempat membaca?

**Skenario C — Replay Event Setelah Bug Fix:**
1. Jalankan producer selama 2 menit (biarkan data terakumulasi)
2. Jalankan `analytics_live.py` — ada "bug" yang sengaja kamu masukkan (hitung rata-rata dengan formula salah)
3. Fix bug-nya
4. Restart consumer dengan `auto_offset_reset=earliest` dan group_id baru
5. Apakah consumer dapat memproses ulang semua data historis?

> Pertanyaan yang harus dijawab: Ini adalah keunggulan Kafka vs sistem message queue tradisional. Jelaskan kenapa fitur ini sangat berharga dalam konteks sistem peringatan bencana!

---

### Panduan Agar Tidak Bisa Dikerjakan Langsung oleh AI

> ⚠️ Perhadikan: tugas ini dirancang agar **kamu harus mengeksekusi kode, mengamati output nyata, dan berpikir kritis** — bukan sekadar menghasilkan kode. Deliverable 3 khususnya tidak bisa diselesaikan tanpa benar-benar menjalankan sistem.

Dalam laporan, **setiap argumen teknis di Deliverable 1 harus didukung oleh**:
- Screenshot output CLI Kafka yang relevan (topic describe, consumer group lag, dll)
- Log dari running sistem kamu (bukan log fiktif)
- Perbandingan sebelum vs sesudah perubahan yang kamu buat

> 💡 **Tanda bahwa arsitek Kafka yang baik:** ia bisa **menjelaskan trade-off**, bukan hanya menyebut angka konfigurasi. Contoh buruk: *"Saya pilih 4 partisi."* Contoh baik: *"Saya pilih 4 partisi karena ada 4 consumer dalam group alerting, sehingga load terdistribusi merata. Replication factor 2 cukup untuk kluster dev (bukan 3) karena ini PoC dan storage terbatas — tapi di produksi harus 3."*
...
| ---- | ------ | -------- |
| Kode pipeline | Repository GitHub/GitLab (public atau shared) | Link di LMS |
| Dokumen arsitektur | `ARSITEKTUR.md` di repo | Di repo |
| Laporan Skenario Uji | `LAPORAN_UJI.md` di repo + screenshot | Di repo |
| Video demo | Max 5 menit, sistem berjalan nyata | Link YouTube/Drive di LMS |

### Rubrik Penilaian

| Dimensi | Bobot | Indikator Kualitas Tinggi |
| ------- | ----- | ------------------------- |
| **Kedalaman Desain Arsitektur** | 30% | Setiap keputusan didukung alasan teknis + pertimbangan trade-off; bukan hanya konfigurasi default |
| **Kualitas Implementasi** | 30% | Kode berjalan nyata, producer realistis, consumer robust (handle restart, error); bukan kode tutorial |
| **Skenario Uji Kegagalan** | 25% | Screenshot nyata dari sistem yang berjalan; analisis jujur tentang apa yang terjadi (termasuk jika ada yang tidak sesuai ekspektasi) |
| **Kelengkapan & Presentasi** | 15% | README yang memungkinkan orang lain menjalankan ulang; video demo yang jelas; kode terstruktur |
| **Bonus: Kafka Streams** | +5% | Gunakan Kafka Streams API (Java) untuk salah satu consumer, bukan hanya poll-loop Python |

---

### Referensi untuk Tugas Kelompok

1. **Kafka 4.2 Documentation** (wajib dibaca): https://kafka.apache.org/42/documentation/
2. **Topic Design Best Practices**: https://kafka.apache.org/42/documentation/#design_filesystem
3. **Producer Configs Reference**: https://kafka.apache.org/42/documentation/#producerconfigs
4. **Consumer Configs Reference**: https://kafka.apache.org/42/documentation/#consumerconfigs
5. **Delivery Semantics**: https://kafka.apache.org/42/documentation/#semantics
6. **Log Compaction**: https://kafka.apache.org/42/documentation/#compaction
7. Narkhede, N., et al. (2017). *Kafka: The Definitive Guide*. O'Reilly Media.
8. Kreps, J. (2013). *The Log*. https://engineering.linkedin.com/distributed-systems/log-what-every-software-engineer-should-know-about-real-time-datas-unifying
